// Generated 2021-01-19 11:42:11 by go-framework v1.2.1-1-ge87b524
module github.com/AtricoSoftware/peg-solitaire

go 1.15

require (
	github.com/atrico-go/console v1.7.0 // indirect
	github.com/atrico-go/container v1.4.0
	github.com/atrico-go/core v1.0.0 // indirect
	github.com/atrico-go/display v0.0.0-20201220180820-e93c3272a166 // indirect
	github.com/atrico-go/viperEx v1.0.0
	github.com/mitchellh/go-homedir v1.1.0
	github.com/spf13/cobra v1.1.1
	github.com/spf13/pflag v1.0.5
	github.com/spf13/viper v1.7.1
)
